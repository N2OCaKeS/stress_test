...
class ServerPresets:
    ...
    def configuring_dhcp(self):
        ...
        ###### TODO TODO TODO !!!!! убрать - test на боевом
        remote_put_file(remote_path="/etc/kea/kea-dhcp4.conf", local_path="kea-dhcp4-test.conf", host=self.host, user=self.user, passwd=self.passwd, port=self.port) 
    ...
    def tune(self):
        remote_cmd("sudo apt install -y kea-dhcp4-server apache2 tftpd-hpa grub-efi", host=self.host, user=self.user, passwd=self.passwd, port=self.port)
        self.change_apache_settings()
        self.configuring_dhcp()
        # remote_cmd()
        remote_cmd("sudo apt install pxelinux && sudo cp /usr/lib/PXELINUX/pxelinux.0 /srv/tftp/", host=self.host, user=self.user, passwd=self.passwd, port=self.port)