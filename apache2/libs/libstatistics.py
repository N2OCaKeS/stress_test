...
from apa_conf import CONFLUENCE_URL
...
class ApacheStatistics:
    ...
    def get_list_required_pages(self, id_root_page):
        ...
        for id_children_from_main_page in children_main_page:
            ...
            for page_id in astra_version_child_pages:
                ...
                """
                if "Apache" in page.get("title") and self.CP.get_child_page_as_html(id=page_id):
                ...
                if "1.7" in name_child_page or "1.8" in name_child_page:
                    ...
                    if hz_kak_nazvat_pages:
                        for item_page in hz_kak_nazvat_pages:
                            if "Apache" in self.CP.get_page_as_html(id=item_page).get("title"):
    ...
    def get_info_from_pages(self, pages, columns_df, rc=False, version_key=None):
        ...
        def collect_data(test_name="apache-rp"):
            data_for_df = {
            ...
            for id in pages:
                ...
                for title in list_pages_with_report:
                    ...
                    if temp_data[0] == test_name:
                        ...
                        """
                        try:
                            if test_name == "apache-rp":
                                rps = soup.find(string='Requests per second').next_element.next_element.text
                        ...
                        if stand == "stand3":
                            if test_name == "apache-rp":
                                rps_with_link = f'<a href="{link}">{rps}</a>'
                                data_for_df["stand3"]["data"].append([astra_version, kernel, sec_mode, stand, rps_with_link, float(rps)])
                            else:
        ...
        def build_dataframes(data_for_df, test_name):
            ...
            for key, data in data_for_df.items():
                ...
                # dataframes_for_summary_graph[key] = df[['Релиз', 'Ядро', 'Стенд', 'Requests per second', 'rating_2']]
        ...
        def create_graphs(data_for_df, test_name, temp_data_for_graph):
            ...
            for key, data_ratings in data_for_df.items():
                ...
                ax.set_title(f"Сравнительная диаграмма количества выполненных запросов в секунду.\n{test_name}")
                ...
                ax.set_ylabel("Количество запросов в секунду")
        ...
        data_df = collect_data(test_name="apache-rp")
        tmp_data_for_gr, tmp_data_krnl, df_psql = build_dataframes(data_for_df=data_df, test_name="apache-rp")
        create_graphs(data_for_df=data_df, test_name="apache-rp", temp_data_for_graph=tmp_data_for_gr)


    """
        Создаем итоговую html страницу для life
    """
    def upload_statistics(self, type_stat='Apache', rc=None, pp_title=None):
        if rc and pp_title:
    ...
    def update_statistics(self):
        ...
        columns = ["Релиз", "Ядро", "Режим защищенности", "Стенд", "Requests per second", 'rating_2']
...
if __name__ == '__main__':
    ...
    stat = ApacheStatistics(username=args.USER, token=args.TOKEN)
