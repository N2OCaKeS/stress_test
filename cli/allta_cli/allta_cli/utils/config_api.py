...
def tokens(tokens_type: Optional[str] = None) -> Union[str, Dict[str, str]]:
    ...
    if not isinstance(data, dict):
        raise ConfigApiError("Неверный формат ответа для tokens.json: ожидается объект.")
...
def files(filename: str) -> Any:
    ...
    try:
        ...
        raise ConfigApiError(f"Файл '{filename}' не является корректным JSON. Фрагмент ответа: {snippet!r}")
