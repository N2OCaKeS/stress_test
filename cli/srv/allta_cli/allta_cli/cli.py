...
@cli.command(
    ...
    help=(
        ...
        "- <имя_сервера> / <номер_стенда> сервера (например, `stand12_srv-main` или `12`);\n"
        "- <API-имя_вм> с префиксом `stand<N>_` (например, `stand6_virtual-station1`);\n"
        "- <номер_стенда_вм> в виде `6` или `stand6` (виртуальный стенд по stand_no).\n"
        "Можно явно указать тип цели:\n"
        "- server:<имя_сервера_или_стенд>\n"
        "- vm:<имя_вм_или_имя_сервера_или_стенд_или_stand_no>\n"
        "- local:<имя_local_vm>\n"
    ),
)
