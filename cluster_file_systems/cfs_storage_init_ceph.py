...
class CephStorageCreate():
    ...
    def create_storage(self):
        ...
        if self.astra_version.startswith("1.7"):
            send_remote_command(f"bash /var/tmp/storage_init/ceph_create_old.sh {self.type_interface_ceph}", ip=self.HOSTS['astra-ceph-admin']['ip'], user="ceph-adm", password="1")
        elif self.astra_version.startswith("1.8"):
            # TODO забирать из hosts
            host_main = "10.0.5.31"
            # type_test = "rbd"
            send_remote_command(f"bash /var/tmp/storage_init/ceph_create_new.sh {host_main} {self.type_interface_ceph}", ip=self.HOSTS['testvm1']['ip'], user="ceph-adm", password="1")
        else:
