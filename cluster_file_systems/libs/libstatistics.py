...
from cfs_conf import CONFLUENCE_URL
...
class FileSystemStatistics:
    ...
    def get_info_from_pages(self, pages, rc=False, version_key=None):
        ...
        def create_summary_table(fs_data=[], stand=None):
            ...
            df_merge['Sort'] = df_merge['Релиз'].apply(lambda s: [LooseVersion(x) for x in s.split('.', 1)])
            df_merge.sort_values(by='Sort', inplace=True)
            df_merge.drop(columns='Sort', inplace=True)
        ...
        for key, data in file_system_data['data'].items():
            ...
            if data.get("OCFS2"):
                rating_for_graph, shcl = build_main_dataframe("OCFS2", data.get("OCFS2"), key)
                build_mat_stat_dataframe("OCFS2", rating_for_graph, key)
                build_graph(fs_type="OCFS2", stand=key, rating_fg=rating_for_graph, shcala_txt=shcl)
