...
| `user.unlock` | WARNING | `user_service.unlock_user` (`POST /users/{id}/unlock`) | user | Admin снял brute-force lockout: `failed_login_attempts` и `locked_until` сброшены. `details.was_locked` отличает реальное снятие от no-op-вызова по незалоченному юзеру, `details.failed_attempts_cleared` — сколько неудач было до сброса. Бан/статус не затрагиваются. |
