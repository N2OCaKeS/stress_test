...
**Всего тестов**: **943 passed** в **65 файлах** под `auth_service/tests/`. Быстрый прогон (`make test-auth`) — ~80–100 сек на свежем стенде. E2E (`make test-auth-e2e`) добавляет ~22 теста и ~2–3 мин (поднимает `auth-service-e2e` + `fetch-cert` + `docker-registry` через профиль compose `e2e`).

Файл фиксирует структуру каталогов и состав групп. Перечень тестовых функций ниже — снимок, точечные имена могут эволюционировать вместе с фичами; для актуальной картины смотреть сам код тестов.
