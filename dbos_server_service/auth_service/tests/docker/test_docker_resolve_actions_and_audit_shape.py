"""Покрытие оставшихся веток docker-registry token flow.

Мелкие, но регрессионно-значимые ветки: push на disabled registry с
не-legacy scope (симметрия с pull-веткой), unit-покрытие `_resolve_actions`
по всем 4 переходам, и точная shape-проверка NO_CFG/DISABLED audit-деталей
(`username`, `service`, `requested_scope`) сверх одного только `reason`.
"""
