"""auth: group_service GET-доступ, redaction-ключи и oauth-edge-кейсы.

Что покрывается:
  - group_service GET endpoints (list_members / list_bot_members /
    list_group_services / list_group_roles) — DA своего отдела имеет
    доступ; cross-dept DA / обычный юзер по-прежнему 403.
  - redaction._HASH_KEYS больше не содержит generic `hash` — git_commit_hash
    и подобные не маскируются.
  - redaction._SECRET_KEYS содержит `service_api_key` и `service_key`.
  - oauth_service.issue_authorization_code — confidential клиент с
    PKCE=plain пишет WARNING.
  - oauth_service.exchange_code → OAUTH_USER_NOT_FOUND при гонке DELETE user.
...
# ── group_service GET endpoints: DA-of-own-dept доступ ──────────────────
...
# ── redaction: generic `hash` больше не маскируется ─────────────────────
...
# ── redaction: service_api_key / service_key в _SECRET_KEYS ─────────────
...
# ── oauth_service PKCE plain WARNING для confidential ────────────────────
...
# ── exchange_code: OAUTH_USER_NOT_FOUND на race с DELETE user ──────────
...
# ── _resolve_actor_dept(actor_id=None) → None ─────────────────────────
...
# ── collect_user_permissions(include_groups=False) ────────────────────
...
# ── reset_password actor_role=None + DA + cross-dept ──────────────────
...
# ── issue_authorization_code: unknown PKCE method для confidential ─────
