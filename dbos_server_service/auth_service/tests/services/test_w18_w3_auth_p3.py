...
2. `_check_can_manage_bot_or_audit.extra_details` — cov gap: при failure
   `extra_details` мёрджится в audit-details.
3. `redaction._classify_value` — закрытие `cs_*` (OAuth client_secret) по
   форме значения, не только по имени ключа.
4. `oauth_service.exchange_code` — нет лишнего commit'а между mark_used и
   audit_emit (dead-code removal, проверяем что код не падает и audit
   пишется как раньше).
5. `bot_ip_tracker.track_bot_ip` — concurrency-замечание в docstring
   зафиксировано (smoke: docstring содержит явное упоминание про N>1).
6. `/oauth2/authorize` — state с `+` round-trip восстанавливается как `+`,
...
async def _make_bot(db, dept_id, name="p3_bot", *, is_active=True, allowed_services=None) -> BotAccount:
    bot = BotAccount(
        id=bot_id(),
        name=name,
...
# ── 1a. assign_roles → inactive user → ConflictError(USER_INACTIVE) ──────────
...
# ── 1b. assign_bot_roles → inactive bot → ConflictError(BOT_INACTIVE) ────────
...
# ── 1c. bulk_assign → inactive user → ConflictError(USER_INACTIVE) ───────────
...
# ── 2. _check_can_manage_bot_or_audit.extra_details merge на failure ─────────
...
# ── 3. redaction._classify_value по `cs_*`-значению ──────────────────────────
...
# ── 4. exchange_code — dead-commit removal smoke ─────────────────────────────
...
# ── 5. bot_ip_tracker — docstring фиксирует concurrency-замечание ────────────
