...
| [02_services.drawio](02_services.drawio) | Карта взаимодействия: клиенты (cli, web_settings, curl, bot) → backend (auth_service, loging_service, server_service, server_worker, config_service) → хранилища (4 PostgreSQL + Redis) → внешние системы (iDRAC, SSH, PXE). Жирные стрелки — синхронный HTTP, пунктир — очередь/событие. |
