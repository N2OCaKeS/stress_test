...
- **Charset validators.** `actor_id` / `target_id` / `department_id` / `request_id` — `^[A-Za-z0-9_\-]{1,48}$` (request_id — до 64); `target_type` — `^[a-z_.]{1,64}$`; `service` / `action` / `username` — свои паттерны (snake_case + точка для `action`, после Unicode-нормализации — `[a-z_]{1,64}` для `service`). Параллельно middleware санирует incoming `X-Request-ID` (стрипает CRLF/NUL, truncate, fallback `req_<hex>`).
...
- **Shadow-keys guard.** `EventCreate.details` запрещает reserved-keys (`actor_id`, `actor_type`, `service`, `action`, `status`, `request_id`, `severity`, `event_id`, `occurred_at`, `department_id`) на любой глубине вложенности — чтобы держатель `SERVICE_API_KEY` не shadow'ил identity actor'а через нестед `details`. NUL-byte в ключах/значениях тоже банится. Walker итеративный — рекурсия сама была бы DoS-вектором.
