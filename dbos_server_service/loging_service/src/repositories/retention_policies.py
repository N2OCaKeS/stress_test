...
def apply_active(
    ...
    *,
    ...
    while True:
        ...
        total += deleted
        # Выходим только когда чанк не удалил НИ ОДНОЙ строки — значит
        # matching row'ов под предикат больше не осталось. Раннее условие
        # `deleted < chunk_size` ломалось под конкурентным sweep'ом: если
        # параллельный проход (ручной admin-прогон вне advisory-lock'а)
        # снёс часть отобранных id между нашим `id_select` и DELETE'ом,
        # rowcount оказывался меньше chunk_size, хотя дальше по `id.asc()`
        # ещё лежали неудалённые подходящие события — мы обрывали sweep
        # раньше времени и оставляли хвост журнала за порогом retention.
        # `deleted == 0` означает, что весь следующий чанк id уже пуст:
        # либо строк не осталось, либо их добил конкурент — в обоих случаях
        # работать больше не над чем.
        if deleted == 0:
            break
    return total
