...
1. redact depth boundary: the xfail from test_cov_loging_w11 wrapped
   ...
   NOT > _MAX_DEPTH, so truncation never fires. Fix: wrap _MAX_DEPTH + 1 times.

2. PATCH /rules/{id} name conflict → 409 RULE_NAME_CONFLICT: replaces the
   monkeypatch-xfail from test_cov_loging_w11 with a real DB-level collision
   (create two rules, rename first to second's name).

3. GET /events ?action= normalisation: removes the xfail from test_cov_loging_w12.
   list_events calls normalize_identifier(action) before the DB query. Uppercase,
   ZWSP-padded and cyrillic-confusable action values are all exercised.
"""
