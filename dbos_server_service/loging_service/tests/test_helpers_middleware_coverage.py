"""Покрытие для loging_service: branch'и helper'ов и middleware.
...
class TestIsUniqueViolationEnumWrappedPgcode:
    ...
    name fallback) покрыты соседними тестами на разбор pgcode. Эта ветка
    раньше зияла дырой.
    """
...
class TestFetchIdentityUnknownSubjectType:
    ...
    Source-уровень покрыт отдельным тестом fallback'а на anonymous, но
    регрессионная защита поведения на полностью замоканном auth_service
    отсутствовала.
    """
