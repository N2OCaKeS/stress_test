"""Тесты под P0/P1 фиксы по loging_service.
    ...
    def test_same_priority_sorted_by_id_asc(self, db):
        ...
        try:
            ...
            rules_data = [
                ...
                ("rul_zzz_w3_tiebreaker", 100),
                ("rul_aaa_w3_tiebreaker", 100),
                ("rul_mmm_w3_tiebreaker", 100),
            ]
            for rid, prio in rules_data:
                db.add(AuditRule(
                    id=rid,
                    name=f"w3-tiebreaker-{rid}",
                    is_active=True,
                    priority=prio,
                    effect="ALLOW",
                    created_at=now,
            ...
            ids = [r.id for r in sorted_rules if r.id.endswith("_w3_tiebreaker")]
            # priority равный → id ASC: aaa < mmm < zzz.
            assert ids == [
                "rul_aaa_w3_tiebreaker",
                "rul_mmm_w3_tiebreaker",
                "rul_zzz_w3_tiebreaker",
            ]
        finally:
            db.execute(
...
# (покрыт `test_cov_loging_w55_p2.py::TestInsertLegacyRowWithoutPayloadHash`)
