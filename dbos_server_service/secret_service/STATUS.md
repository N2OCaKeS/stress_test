...
- **Phase 5 — access policy + repos.** Полный набор scope-зависимых проверок (`personal` / `department` / `cross_department`), reader/operator vs admin secret_service'а своего dept'а vs account_admin (admin per-(dept, service); cross-dept привилегий нет — admin dep_A не лезет в cred'ы dep_B), базовая последовательность `dep_service_access → status → scope check`. Репозитории credentials / role_acls / dept_grants с курсорной пагинацией.
