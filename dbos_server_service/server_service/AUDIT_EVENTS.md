...
| `server.users_inventory_triggered` | INFO | POST `/api/server/v1/servers/{id}/users/inventory` — dispatch инвентаризации OS-пользователей (SSH getent). target=server: kick'ается со стороны сервера, аккаунты — только результат | `server` | `task_id`, `task_kind=users.inventory` |
