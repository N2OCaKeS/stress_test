...
1. **servers** — hostname/ip/mgmt_ip/ssh_port unique, FK `os_version_id`, `department_id`, `status`, `power_state`, busy-state, `decommissioned_at`. CPU-данные хранятся inline: `cpu_brand` / `cpu_model` / `cpu_cores` / `cpu_threads` / `cpu_frequency_ghz` (обновляются inventory probe'ом или вручную через `PATCH /servers/{id}`). Управление: `is_managed`, `management_user`, `prepared_at` — заполняются после `prepare`-бутстрапа.
