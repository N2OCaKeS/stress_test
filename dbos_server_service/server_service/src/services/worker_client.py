...
async def _fetch_task_status_and_meta(task_id_value: str) -> dict | None:
    ...
    Возвращает dict со status / target_server_id / task_kind / created_by /
    target_resource_id либо None, если row не существует. `created_by` нужен
    cancel-эндпоинту для отличения системных task'ов (heartbeat/sweep/
    cleanup_completed — created_by IS NULL, target_server_id IS NULL) от
    пользовательских: системные требуют отдельной гарды по платформенной роли.
    `target_resource_id` — id доменного ресурса задачи (для `vm.create` это
    id ВМ): cancel'у он нужен, чтобы снять placeholder-строку ВМ при отмене
    ещё не стартовавшего create'а.
    """
    session_factory = _engine_factory()
    async with session_factory() as session:
        row = (
            await session.execute(
                text(
                    ...
                    "target_resource_id FROM tasks WHERE id = :id"
                ),
                {"id": task_id_value},
            )
        ).first()
...
async def list_tasks(
    ...
    * `server_ids` непустой → `target_server_id IN (...)`; при `include_infra`
      ...
      (инфра-задачи без сервера — видны только service-роли admin).

    `created_by` (если задан) дополнительно сужает выборку до задач, которые
    поставил сам caller — для непривилегированного носителя кастомной роли,
    который видит только свои задачи в рамках своего отдела.
