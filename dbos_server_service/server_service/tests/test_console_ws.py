...
Покрытие:
  ...
  * сервер не prepared (`is_managed=False`) → close 4409 PREPARE_REQUIRED + audit;
  * decommissioned → close 4409 SERVER_DECOMMISSIONED;
  * cross-dept / не найден → close 4404;
  * нет токена → close 4401;
  * happy-path bridge: после `ready` ввод клиента публикуется в
    `console:in:<sid>`, вывод из `console:out:<sid>` уходит клиенту,
    на disconnect публикуется `stop`.
...
auth/RBAC/visibility/Redis на уровне модуля `console`. Это упражняет ровно
тот же код RBAC/prepared-gate/моста.
"""
...
class FakeWebSocket:
    ...
    def __init__(self, *, headers=None, subprotocols=None, incoming=None):
...
@pytest.mark.asyncio
async def test_not_prepared_closes_prepare_required(monkeypatch, _patch_console):
    async def allow(*a, **k):
        return None
    ...
    assert any(
        ...
        and e.get("details", {}).get("reason") == "prepare_required"
        for e in _patch_console
    )
...
async def test_bridge_publishes_input_and_relays_output(monkeypatch, _patch_console):
    ...
    assert start_obj["management_user"] == "dbos"
    ...
    # session_open + session_close в аудите.
