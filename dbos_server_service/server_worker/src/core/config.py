...
class Settings(BaseSettings):
    ...
    # УСТАРЕЛО (не читается воркером). Управляющая SSH-пара теперь своя на
    # каждом сервере: публичный ключ генерит server_service и присылает в
    # prepare-stash (`mgmt_install`) / fan-out payload, а не берётся из
    # глобального env. Поле оставлено как заглушка до его удаления отдельной
    # инфра-волной; код per-server creds его не использует.
    ssh_management_public_key: str = Field(
        default="",
        description=(
            "DEPRECATED, no longer read by the worker. The management public "
            "key is now per-server, generated by server_service and delivered "
            "via the prepare stash (mgmt_install) / fan-out payload."
        ),
    )
    # УСТАРЕЛО (не читается воркером). Приватный ключ управляющего пользователя
    # тоже per-server: воркер тянет его just-in-time через
    # `fetch_management_credentials` (для prepare — из mgmt_install). Глобальный
    # mounted-secret больше не используется; поле — заглушка до удаления.
    ssh_management_private_key_path: str = Field(
        default="",
        description=(
            "DEPRECATED, no longer read by the worker. Per-server management "
            "private keys are fetched just-in-time from server_service "
            "(fetch_management_credentials); prepare uses mgmt_install."
        ),
    )
