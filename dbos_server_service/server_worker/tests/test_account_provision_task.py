...
class TestProvisionTaskWithInlineCreds:
    ...
    async def test_inline_secrets_scrubbed_after_success(
        ...
        """После успешного provision plaintext password и private SSH key
        в `tasks.payload` маскируются на `<scrubbed>`, public key и прочие
        поля остаются на месте — оператор с SELECT'ом на worker.tasks не
        получит секреты, даже пока row живёт до retention cleanup'а.
        """
        ...
        tid = await make_task(
            task_kind="account.provision", target_server_id="srv_scrub",
            payload={
                "server_id": "srv_scrub", "account_id": "acc_scrub",
                "login": "ops",
        ...
        # Открытый ключ и прочие неконфиденциальные поля остались.
        assert t.payload.get("ssh_public_key") == _ED25519_PUB
        assert t.payload.get("server_id") == "srv_scrub"
        assert t.payload.get("account_id") == "acc_scrub"
