"""Точечные сценарии вокруг `_write_authorized_key`,
`_install_authorized_key`, `_sudo_group_membership` и audit-маски
`installed_packages.pattern`.

  * `_write_authorized_key` через `create_user(force_replace=True)`:
    existing user, force_replace=True → bash-команда содержит overwrite
    (`>` без `>>`), ключ уходит на stdin.
  * `_write_authorized_key` напрямую (existing user path): stdin
    содержит ровно ключ, не команду.
  * `_install_authorized_key` принимает `target_home` через
    `create_user(home_dir=...)`: системный home отбивается до SSH.
  * `_sudo_group_membership` `id -nG` rc!=0 → пустое множество (юзер
...
_PUBKEY = (
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKeyForCoverage "
    "authkey@test"
)
...
# ══════════════════════════════════════════════════════════════════════════════
# create_user(force_replace=True) на существующем юзере → bash с `>`,
...
# ══════════════════════════════════════════════════════════════════════════════
# _sudo_group_membership: id -nG rc!=0 → пустое множество
# ══════════════════════════════════════════════════════════════════════════════
