"""Точечные сценарии по cancel-metadata, BMC-dispatch'у и reencrypt-tick'у.

  * _attach_cancel_metadata unit: task=None (early return), задача без
    поля (getattr fallback), все три поля заполнены → details.
  * _wrap_ipmitool_error: returncode=0 с незнакомым stderr → BMC_ERROR;
    "username"/"password is incorrect" stderr → BMC_AUTH_FAILED;
    rc=-1 + "not found" НЕ in message (другой -1) → stderr-маршрут.
  * dispatch_power_action / dispatch_get_power_state / dispatch_rotate_user_password:
  ...
  * secrets_reencrypt_lazy: non-string outbox_id в claim-результате → errors+1;
    seed_failure (CredentialFetchError) → ранний return без raise;
    unexpected exception в seed → ранний return.
  * midrun-cancel без cancelled_at — timestamp НЕ должен быть в audit, когда
    cancelled_at=None в свежем refresh.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# _attach_cancel_metadata — unit-level
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# _wrap_ipmitool_error — непокрытые ветки
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# secrets_reencrypt_lazy — непокрытые ветки main.py
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# midrun-cancel без cancelled_at
# ══════════════════════════════════════════════════════════════════════════════
...
class TestMidrunCancelWithoutCancelledAt:
    ...
    Фиксируем фактическое поведение: runner делает fresh SELECT при
    success_cancelled_midrun, и если cancelled_at=NULL — timestamp в audit
    НЕ добавляется.
    """
    ...
    async def test_success_midrun_cancel_null_cancelled_at_no_timestamp(
        ...
        # отдельно в test_runner_cancel_audit_metadata.py.
