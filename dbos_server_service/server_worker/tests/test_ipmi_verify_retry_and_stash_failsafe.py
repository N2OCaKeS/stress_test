"""Verify-retry ротации IPMI, редакция KV-секретов и fail-safe stash-ридеры.
...
async def test_ipmi_verify_retries_once_on_transient_auth_fail(
    ...
    tid = await make_task(
        ...
        target_server_id="srv_verify",
        payload={"server_id": "srv_verify"},
    )

    async def fake_fetch(server_id, target_department_id=None):
        return {
            "controller_id": "ipm_verify",
            "kind": "idrac",
