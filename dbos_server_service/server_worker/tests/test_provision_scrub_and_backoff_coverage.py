"""Точечные сценарии по provision-guard'ам, backoff'у и DLQ.

  * _FORBIDDEN_HOMES: Python-guard (target_home=) через _write_authorized_key
    и bootstrap; подтверждение, что ssh_public_key НЕ в scrub-листе provision.
  * backoff cap: граничный случай attempts == _BACKOFF_EXPONENT_CAP (=16),
    attempts == cap-1, cap+1; поведение `_compute_backoff_delay` вблизи _RETRY_MAX.
  * audit_client DLQ: double-prefix guard в `_send_to_dlq` — повторный вызов
    на строке с уже существующим [DLQ:...] prefix'ом не добавляет второй;
    truncate at LAST_ERROR_MAX_LEN с prefix'ом.
  * login guard: managed-ветка с is_managed=True, login задан, но
    apply_session_hints пробрасывает host из payload.
  * ssh_private_key stash: структурная проверка scrub-листа в account_provision
    (содержит оба ключа, но НЕ ssh_public_key).
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# _FORBIDDEN_HOMES: python-level guard через _write_authorized_key и
#        bootstrap; ssh_public_key вне scrub-листа
# ══════════════════════════════════════════════════════════════════════════════

_PUBKEY = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIProvKey provision@test"
...
# ══════════════════════════════════════════════════════════════════════════════
# backoff cap: граничные значения _BACKOFF_EXPONENT_CAP
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# audit_client DLQ: double-prefix guard и truncation в _send_to_dlq
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# login guard — _account_creds: apply_session_hints пробрасывает host
...
class TestAccountCredsHostPropagation:
    ...
    async def test_managed_creds_propagate_host_from_payload(self, monkeypatch):
        ...
        creds = await _account_creds(
            ...
            "srv_managed", "acc_managed", target_dept=None,
        )
    ...
    async def test_self_session_carries_host_from_payload(self, monkeypatch):
        ...
        creds = await _account_creds(
            ...
            "srv_self", "acc_self", target_dept=None,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
# ssh_private_key stash: структурная проверка, что оба ключа в scrub-листе
