"""Разнородные проверки worker'а: валидаторы, docstring'и и breaker-state.

Файл собирает несколько слабо связанных точек в один сьют:
UUID-валидация в HTTP-клиенте, Python-guard для запретных home'ов в SSH,
docstring redfish'а, pure-read для breaker'а, SKIP-LOCKED-комментарий и
docstring clock-skew.
