...
``FakeRedis`` (общий с audit_publisher_breaker — модуль
`_breaker_test_helpers`) с in-memory dict'ом + py-репликой Lua-скриптов.
Тест проверяет state-machine: closed→open, open-rejects, half_open→closed
