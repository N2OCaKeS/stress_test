...
# (помечается как throttled, но выдачу не блокирует)`,
      notes:
        "Ответ 200: { login: str | null, secret_b64: \"<base64(plaintext)>\" }. Повторный reveal в 5-мин окне помечается как throttled, но HTTP всё равно 200 с секретом. Нет права на read → 403 CREDENTIAL_ACCESS_DENIED. Кред'а не найдена / cross-dept miss → 404 CREDENTIAL_NOT_FOUND. status=blocked → 410 CREDENTIAL_BLOCKED (blocked_reason/blocked_at в details). Reveal до valid_from → 410 SECRET_NOT_YET_VALID; после valid_to → 410 SECRET_EXPIRED. Битый ciphertext/AAD → 422 DECRYPT_FAILED; внутренняя ошибка → 500 DECRYPT_INTERNAL_ERROR; ключ версии шифротекста не в env → 503 ENCRYPTION_KEY_MISSING.",
    },
    {
      id: "transfer",
