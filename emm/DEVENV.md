...
## Примеры логов тестирования

На запущенном dev-стеке выполните `python scripts/seed_test_logs.py` из каталога `emm`. Нужны актуальные миграции и существующий пользователь `dep_admin1`. Скрипт добавляет 126 явно помеченных искусственных попыток (64 в кампании, 62 одиночных), включая ошибку с успешным повтором и debug. Для каждой — четыре сегмента лога с русским текстом и замаскированным примером секрета. Все попытки завершены, обращения к тестовым стендам и внешним интеграциям не выполняются. Повторный запуск не меняет существующие записи.

Откройте `http://localhost:5173/testing/logs` под `dep_admin1` (стандартный dev-пароль `1`). Для запуска стека с сохранением данных используйте `make up-keep`: `make up` очищает тома.
