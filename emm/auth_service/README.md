...
- **Service-to-service** — `/authorization/*` закрыты `SERVICE_API_KEY` + опциональный `X-Service-Identity` (soft / strict через `STRICT_SERVICE_IDENTITY`). `SERVICE_API_KEYS` JSON env даёт per-service ключи в дополнение к shared. **В prod рекомендуется**: заполнить `SERVICE_API_KEYS` (per-service map) + `STRICT_SERVICE_IDENTITY=true`. Дефолт `STRICT_SERVICE_IDENTITY=false` оставлен для backward-compat (legacy-клиенты без `X-Service-Identity` продолжают работать), но в этом режиме неизвестный header пишется WARNING'ом и не отбивается — миграция на strict + per-service map обязательна для production.
