...
class Settings(BaseSettings):
    ...
    # проигравший наивно ретраит СТАРЫМ токеном (нового у него на руках нет),
    # его hash уже лежит в истории ротаций — без окна это трактуется как reuse
    # и сносит все сессии юзера. В пределах этого окна повтор непосредственно-
    # предыдущим hash'ем считается benign-гонкой (RACE, сессия жива), а не
    # атакой. Настоящий reuse — старый токен вне окна либо не последний в
    # истории — по-прежнему ловится и ревокает сессию/цепочку.
    # 0 или меньше — окно выключено (любой ротированный hash = reuse).
    refresh_race_grace_seconds: int = Field(
        default=20,
        alias="REFRESH_RACE_GRACE_SECONDS",
        description=(
            "Сколько секунд после ротации повтор непосредственно-предыдущим "
            "refresh-hash считается benign-гонкой, а не reuse. 0 — выключено."
        ),
    )
