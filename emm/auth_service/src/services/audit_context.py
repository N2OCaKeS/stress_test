...
#       in sync with sdk/extract_client_ip.py и server_service/src/services/audit_context.py.
"""Per-request audit-контекст. Заполняется middleware, читается `audit_service.emit`.
...
contextvar изолирован по asyncio task — fastapi обрабатывает каждый запрос в своём
task, так что одновременные запросы не пересекаются. Для sync-кода (фоновые
потоки в `asyncio.to_thread`) контекст наследуется автоматически.

Также содержит helper `extract_client_ip(request)` — безопасную замену
наивного парсинга `X-Forwarded-For`. Заголовку доверяем ТОЛЬКО если запрос
пришёл с IP из `Settings.trusted_proxy_ips`. Иначе берём прямой
`request.client.host`. Это закрывает spoofing-вектор: без allow-list
любой клиент мог поставить `X-Forwarded-For: 1.2.3.4` и подделать actor IP
в audit-log / обойти rate-limit-by-IP.
"""
...
class AuditContext:
    ...
    # `subject_type` зеркалит `IdentityContext.subject_type` (диспатч JWT по
    # `actor_type`). Без него audit писал бы всё как `actor_type="user"` —
    # PAT/bot/oauth-токены смешались бы с человеческими действиями.
    # `audit_service.emit` подхватывает поле как default actor_type, если
    # caller не задал явно. Значения: "user"/"bot"/"oauth_client"/"pat"/None
    # (анонимный middleware-вход до identity resolve).
...
def set_context(ctx: AuditContext) -> object:
    """Установить контекст текущего запроса. Возвращает token для последующего reset()."""
