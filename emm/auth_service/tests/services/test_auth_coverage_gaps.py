"""auth_service: точечные ветки, оставшиеся непокрытыми.

Целевые ветки:
  - bot.token_create failure-audit содержит token_name в details
  - redact({"bot_token": ["list"]}) → контейнер под секретным ключом целиком
  - oauth state с %-литералом — double-encode (% → %25)
  - bot_ip_tracker: bot_suspicious_ip_window_seconds читается из settings
    (монопатч на cached settings работает)
  - bot_roles_purge: purge эмитится безусловно, если removed_services непустой
  - downgrade: динамический resolve revision вместо хардкода
  - refresh_session_ip: обновление через patch extract_client_ip
...
# ── redact — контейнер под секретным ключом ───────────────────────────
...
# ── bot.token_create failure содержит token_name в details ─────────────
...
class TestBotTokenCreateFailureDetails:
    async def test_cross_tenant_failure_audit_contains_token_name(
        ...
        dept_admin_b = User(
            id=_new_id("usr_"),
            username="token_name_dept_admin_b",
            password_hash=hash_password("DeptAdminB1!"),
...
# ── oauth state с %-литералом → double-encode ─────────────────────────
...
class TestBotRolesPurgeBehaviorConfirmed:
    async def test_narrowing_with_no_roles_still_emits_purge_when_service_removed(
        ...
        """
        captured = _capture_audit(monkeypatch)
        await _grant_service_to_dept(db, dept_a_with_service.id, "svc_no_roles")
        await db.commit()
        ...
        bot_resp = await client.post(
            ...
            json={
                ...
                "allowed_services": [service_x.service_name, "svc_no_roles"],
            },
        )
        ...
        assert "svc_no_roles" in details["removed_services"]
...
class TestAuditServiceOverflowCapAPI:
    ...
    async def test_overflow_counter_resets_to_zero(self, monkeypatch):
        ...
        audit_service.emit("user.login", request_id="req_a")
        await asyncio.sleep(0)
        audit_service.emit("user.login", request_id="req_b")
        await asyncio.sleep(0)
