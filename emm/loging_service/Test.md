...
**Всего тестов**: точное число живёт в выводе `make test-logging`. Грубая оценка по `grep -c 'def test_' tests/*.py` даёт ~1380 функций; реальный pytest-итог выше за счёт `@pytest.mark.parametrize` (один `def test_*` раскладывается на N тестов на collection-стадии). Канонический счётчик — `pytest --collect-only -q | tail -1`, без него любая цифра в этом файле быстро устаревает.
**Файлов**: точное число — `ls tests/test_*.py | wc -l` (карри-cover волны добавляли новые регрессионные файлы без сноса старых; на момент правки реестра — порядка 70).
