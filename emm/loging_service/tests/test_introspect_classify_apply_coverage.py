...
Восемь edge-case'ов, оставшихся в углах после основных cov-проходов:
  ...
  2. COUNT-side `_with_statement_timeout` с pgcode через `.value`-атрибут
     (psycopg2 enum-обёртка); SELECT-side покрыт отдельно, COUNT — нет.
...
class TestCountStatementTimeoutViaValueAttr:
    ...
    `getattr(orig.pgcode, "value", None)`. SELECT-side покрыт отдельно, COUNT
...
class TestApplyActiveMultiPolicyOr:
    ...
    def test_disjoint_policies_count_unique_rows(self, db):
        ...
        #   policy_crit: retain_days=10, severity=CRITICAL (все сервисы)
        #   policy_auth: retain_days=10, service=auth_service (все severity)
        # События:
        #   E1: auth_service / CRITICAL / 30 дней назад — матчит обе политики
        #   E2: auth_service / INFO     / 30 дней назад — только policy_auth
        #   E3: server_service / CRITICAL / 30 дней назад — только policy_crit
