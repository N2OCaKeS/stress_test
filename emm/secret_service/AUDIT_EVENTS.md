...
| Action | Status | Severity | Payload (`details`) |
|---|---|---|---|
| `tokens.role_acl_added` | success | INFO | `{ cred_id, acl_id, dept_id, role_name, can_view, can_read, can_write }`. |
...
| `tokens.user_acl_added` | success | INFO | `{ cred_id, user_id, can_view, can_read, can_write }` (target_id = acl_id). |
