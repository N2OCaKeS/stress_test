...
| Access policy + repos (Phase 5) | `tests/unit/test_access_service.py`, `test_repositories_credentials.py`, `test_repositories_role_acls.py`, `test_repositories_dept_grants.py` | ~66 | scope-зависимые проверки (`personal` / `department` / `cross_department`), reader/operator vs admin secret_service'а своего dept'а vs account_admin (admin per-(dept, service), cross-dept привилегий нет), last-resort `SERVICE_NOT_AVAILABLE_FOR_DEPARTMENT` / `CREDENTIAL_BLOCKED` / `CREDENTIAL_ACCESS_DENIED`, repo CRUD + курсорная пагинация + filter'ы. |
...
- **Token-фабрики** — `make_token`, `account_admin_token`, `service_admin_token_a` (admin secret_service'а в dep_a), `dep_admin_token_a`, `reader_token_a`, `operator_token_a` (и `_b` вариант для cross-dep сценариев). Имя `service_admin_token_*` сохранено для совместимости фикстур; внутри это admin per-(dept, service), а не платформенная роль.
