...
| `server.node_exporter_installed` | WARNING | Отправка установки node exporter на сервер | `server` | `task_id`, `department_id` |
| `vm.node_exporter_installed` | WARNING | Отправка установки node exporter на ВМ | `vm` | `task_id`, `department_id` |
...
| `server.management_creds_rotated` | CRITICAL | (a) POST `/servers/{id}/management-credentials/rotate` — dispatch ротации (`server.rotate_management_creds`): новый материал в `mgmt_*`, старый в `previous_mgmt_*`, `mgmt_creds_pending_apply=True`; (b) POST `/internal/servers/{id}/management-credentials/applied` — applied-callback воркера снял pending, занулил previous, проставил `mgmt_creds_rotated_at` | `server` | (a) `task_id`, `task_kind=server.rotate_management_creds`, `department_id`, `idempotent_hit`. denied/failure: `reason in {no_view_permission, not_found_or_cross_dept, decommissioned, vanished, prepare_required, rotation_pending, creds_store_unavailable, creds_store_failed, idempotent_conflict, worker_unreachable}` (плюс отбой по брони/обновлению — через отдельные `server.reservation_denied` / `server.astra_update_locked`). (b) `rotated_at`, `caller_type`, `department_id`. denied/failure: `reason in {permission_denied, server_not_found, target_department_mismatch}` |
...
| `server.acquired_for_service` | INFO | POST `/internal/servers/{id}/acquire-for-service` — бронь от имени сервиса (`busy_actor_type=service`, `busy_service_name` из провалидированной `X-Service-Identity`) | `server` | `service_name`, `department_id`, `busy_state`, `busy_note`. failure: `reason in {not_found, decommissioned, decommissioned_race, vanished_during_acquire, already_busy}`; при already_busy — `takeover` (запрошен ли) |
| `server.reservation_taken_over` | WARNING | POST `/internal/servers/{id}/acquire-for-service` с `takeover=true` — сервис отнял бронь `busy` / `testing_done` у прежнего держателя (вместо `server.acquired_for_service`) | `server` | `service_name`, `department_id`, `busy_state`, `busy_note`, `previous_busy_state`, `previous_busy_user_id`, `previous_busy_service_name`, `previous_busy_note` |
| `server.released_for_service` | INFO | POST `/internal/servers/{id}/release-for-service` (busy_state → free) либо `/release-for-service-as-done` (busy_state → testing_done, busy_actor_type/busy_service_name/busy_note сохраняются как контекст) — сервис снял собственную бронь | `server` | `service_name`, `department_id`, `previous_state`, `mark_as` (только у as-done варианта). denied/failure: `reason in {not_found, not_busy, reservation_held_by_other}` |
...
| `vm.acquired_for_service` | INFO | POST `/internal/vms/{id}/acquire-for-service` — бронь ВМ от имени сервиса: `vms.service_busy_state`, `busy_service_name` из `X-Service-Identity`, `status` → `run test` | `vm` | `service_name`, `department_id`, `busy_state`, `busy_note`. failure/denied: `reason in {not_found, target_department_mismatch, vanished_during_acquire, already_busy}`; при already_busy — `current_state`, `takeover` |
| `vm.reservation_taken_over` | WARNING | То же с `takeover=true` — сервис отнял бронь человека (`status=<login>`) или `testing_done` | `vm` | `service_name`, `department_id`, `busy_state`, `busy_note`, `previous_busy_state`, `previous_busy_service_name`, `previous_busy_note` |
| `vm.released_for_service` | INFO | POST `/internal/vms/{id}/release-for-service` (→ `free`) либо `/release-for-service-as-done` (→ `testing_done`, снимает человек через `POST /vms/{id}/release`) | `vm` | `service_name`, `department_id`, `previous_state`, `mark_as`. denied/failure: `reason in {not_found, not_busy, reservation_held_by_other}` |
| `vm.service_status_changed` | INFO | POST `/internal/vms/{id}/service-status` — смена стадии своей брони ВМ (`acs` → `testing`) | `vm` | `service_name`, `department_id`, `previous_state`, `busy_state`, `busy_note`. denied/failure: `reason in {not_found, not_busy, reservation_held_by_other}` |
| `vm.prepare_for_test` | WARNING | Задача воркера `vm.prepare_for_test`: откат снимка ВМ на hub'е, ожидание гостя, учётка теста, ядро, режим, настройка стенда, перезагрузка | `vm` | `vm_id`, `prepare_request_id`, `snapshot_name`, `kernel`, `mode`, `succeeded` |
| `vm.stand_setup` | WARNING | Задача воркера `vm.stand_setup`: настройка ВМ-стенда без отката снимка (параметры ядра, скрипт, перезагрузка) | `vm` | `vm_id`, `stand_setup_request_id`, `succeeded` |
| `settings.vm_test_updated` | WARNING | PUT `/settings/vm-test` — account_admin изменил шаблоны имени снимка ВМ для отката перед тестом | `settings` | `snapshot_name_templates`, `previous` |
| `server.acknowledge_testing_done` | INFO | POST `/servers/{id}/acknowledge-testing-done` — любой пользователь с `(server, *, view)` снял `testing_done` → `free` | `server` | `department_id`. denied/failure: `reason in {not_found_or_cross_dept, permission_denied, not_testing_done}` |
...
| `server.prepare_for_test_requested` | WARNING | POST `/internal/servers/{id}/prepare-for-test` — testing_service запросил подготовку стенда под тест (ACS restore + auto-prepare + провижн тестового пользователя + смена ядра + смена режима безопасности + ребут); то же для ВМ — POST `/internal/vms/{id}/prepare-for-test` (откат снимка ВМ вместо ACS restore), `target_type=vm` | `server` / `vm` | `prepare_request_id`, `correlation_id`, `os_version_id`, `kernel`, `mode`, `preparation`, `skip_pam_fix`, `test_username`, `test_account_credential_id`, `service_name`, `department_id`; у ВМ — `target_type=vm` |
| `server.prepare_for_test_completed` | WARNING | Пайплайн `prepare-for-test` дошёл до терминального состояния (входная валидация, либо worker-callback `POST .../prepare-for-test-done`; у ВМ — `/internal/vms/{id}/prepare-for-test-done`) | `server` / `vm` | `prepare_request_id`, `correlation_id`, `succeeded`, `failed_step in {restore, vm_revert, prepare, user_provision, pam_fix, stand_setup, kernel_change, mode_switch, reboot_verify}`, `kernel`, `os_version_id` |
| `server.stand_setup_requested` | WARNING | testing_service запросил настройку стенда без restore (`POST .../stand-setup`; у ВМ — `/internal/vms/{id}/stand-setup`) | `server` / `vm` | `stand_setup_request_id`, `correlation_id`, `service_name`, `department_id`, `kernel_cmdline_extra`, `has_script` |
| `server.stand_setup_completed` | WARNING | Настройка без restore завершилась (callback воркера или провал диспатча) | `server` / `vm` | `stand_setup_request_id`, `correlation_id`, `succeeded`, `failed_step in {pam_fix, stand_setup, reboot_verify}` |
| `server.stand_setup` | WARNING | Задача воркера `server.stand_setup` | `server` | `server_id`, `stand_setup_request_id`, `succeeded` |
...
| `server.astra_update` | WARNING | POST `/servers/{id}/astra-update` — dispatch обновления ОС (`server.astra_update`: перезапись sources.list из `OsVersion.repositories` + `apt update && astra-update`). Ставит `busy_state='updating'`, max_attempts=1 | `server` | success: `task_id`, `task_kind`, `os_version_id`, `os_version_name`, `repositories_count`, `department_id`, `idempotent_hit`. failure: `reason in {not_found_or_cross_dept, decommissioned, vanished, os_version_not_found, os_version_no_repositories, idempotent_conflict, worker_unreachable}` |
| `server.astra_updated` | WARNING | POST `/internal/servers/{id}/astra-updated` — callback воркера об исходе обновления ОС (снимает updating-блокировку; на успехе привязывает `os_version` и триггерит inventory) | `server` | `succeeded`, `os_version_id` (привязанная / None), `requested_os_version_id`, `department_id`, `caller_type`. denied/failure: `reason in {permission_denied, server_not_found, target_department_mismatch}` |
| `server.astra_update_locked` | WARNING | Операция над сервером отбита, потому что идёт обновление ОС (`busy_state='updating'`) — 409 SERVER_UPDATING для всех, включая владельца брони и админа | `server` | `blocked_action`, `server_id`, `department_id`, `busy_note`, `subject_type` |
| `server.astra_update_recovered` | WARNING | Плановый sweep (в `/internal/servers/auto-inventory-sweep`) освободил сервер, зависший в `busy_state='updating'` дольше TTL (`ASTRA_UPDATE_STUCK_TTL_MINUTES`): callback `astra-updated` не пришёл (воркер упал/потерял задачу). Блокировка снята в `free`; actor_type=system | `server` | `reason=stuck_updating_ttl`, `department_id`, `ttl_minutes`, `stuck_minutes` |
...
| `server_account.ssh_key_set` | WARNING | POST `/server-accounts/{id}/ssh_key` — SSH-ключ задан/заменён (`generate` Ed25519 или `supply` public); сохранён в БД и расходится через `account.update_on_host` (push authorized_keys). Гейт — `update` | `server_account` | `login`, `ssh_mode in {generate, supply}`, `department_id` |
| `server_account.ssh_key_rotate` | CRITICAL | POST `/server-accounts/{id}/rotate_ssh_key` — Ed25519-ключ перегенерён (кейс компрометации): новая пара сохранена, прежний приватный удержан в `previous_ssh_private_key_encrypted`, приватный отдан один раз, расходится через `account.update_on_host`. Гейт — `update` | `server_account` | `login`, `department_id` |
| `server_account.ssh_key_generated` | INFO | SSH-пара аккаунта авто-сгенерирована при резолве кред (prepare/provision), потому что у учётки ключа не было; public уходит в authorized_keys | `server_account` | `login`, `department_id`, `ssh_key_fingerprint` |
| `server_account.apply_credentials` | WARNING | Пароль+ssh-ключ учётки проброшены на привязанные серверы через `account.update_on_host` (ручной POST `/server-accounts/{id}/apply` или авто после set/rotate пароля/ключа). Гейт — `rotate_password` | `server_account` | `login`, `department_id`, `dispatched`, `skipped`, `source=manual_apply` |
| `server_account.reveal_previous_ssh_private_key` | CRITICAL | GET `/server-accounts/{id}/previous_ssh_private_key` вызывающим с `view_password` — удержанный ПРЕЖНИЙ приватный SSH-ключ расшифрован и отдан в PEM (доступен на время переходного периода ротации). failure: `reason in {no_previous_ssh_key_stored, decrypt_failed}` | `server_account` | `login`, `department_id` |
| `server_account.clear_previous_ssh_key` | INFO | DELETE `/server-accounts/{id}/previous_ssh_private_key` вызывающим с `rotate_password` — удержанный ПРЕЖНИЙ приватный SSH-ключ занулён вручную (переходный период закрыт). Идемпотентно: no-op, если удержанного ключа не было | `server_account` | `login`, `department_id`, `cleared` |
