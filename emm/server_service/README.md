...
- `os_versions` — write-CRUD под матрицей; чтение каталога (list / get по id / get по имени) — публичное, без auth. Поле `repositories` (список URL). OS-каталог **глобальный для всей платформы** (все департаменты в одной компании): `_resolve_or_create_os` в `src/services/internal_service.py` использует `name` как unique key без `department_id`, одна OS-запись на все департаменты. Решение владельца 2026-05-30: компания одна, OS общие. Inventory-callback worker'а на ранее не виденное имя ОС создаёт запись и поднимает WARNING-аудит `os_version.create` (`reason=auto_from_inventory`). Если в будущем понадобятся per-dept OS-конфиги — миграция на composite key `(name, dept_id)` + индекс и правка `osv_repo.get_by_name`.
...
- `prepare` — бутстрап управления: `POST /servers/{id}/prepare` (bootstrap-креды в base64) → worker заводит управляющего пользователя `dbos` + кладёт management SSH-ключ; на сервере выставляются `is_managed` / `management_user` / `prepared_at`. Bootstrap-пароль проверяется усиленной политикой: минимум 16 символов, буква, цифра и символ (нарушение → 422 с `type=WEAK_PASSWORD`).
...
Документ-источник — `1.txt`, раздел `server_service`, 12 пунктов. Текущая
реализация:
...
12. **Контракт ошибок.** Единый JSON-envelope:
    ...
    `NO_IPMI_CONTROLLER`, `TASK_IDEMPOTENT_CONFLICT`, `WORKER_UNREACHABLE`,
...
| `ASTRA_UPDATE_STUCK_TTL_MINUTES` | порог зависшей OS-update блокировки: если сервер держит `busy_state='updating'` дольше этого (по `busy_since`), а callback `astra-updated` не пришёл, плановый sweep освобождает его в `free` и эмитит `server.astra_update_recovered`; default `60` |
...
Тесты проходят зелёным. Точное число динамически растёт по мере появления новых регрессионных и unit-сьютов — актуальная цифра в `../TEST_COVERAGE.md`. Запуск:
