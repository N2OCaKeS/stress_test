"""Строгий декод base64-кред на входе write-эндпоинтов.

Reveal-карты (server_account / ipmi_controller) отдают plaintext-секрет в
`password_b64` через `base64.b64encode`. Симметрично write-контракт принимает
секрет в base64: клиент кодирует `base64.b64encode(plaintext)`, мы декодируем
на приёме и дальше работаем с plaintext (политика, шифрование, хранение).
"""
