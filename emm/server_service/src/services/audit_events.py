...
SERVICE_EVENTS = [
    ...
    {"action": "http.platform_admin_blocked", "description": "Platform admin (account_admin + loging_admin) blocked from server_service business endpoint (admin-plane separation; loging_reader is NOT blocked)", "default_severity": "WARNING"},
    # Servers — CRUD
    ...
    {"action": "settings.host_services_updated", "description": "department_admin/service admin изменил настройки SSH-доступа к своему отдельскому хосту для host-service control (host/port/user/приватный ключ через PUT /settings/host-services)", "default_severity": "INFO"},
    {"action": "settings.host_service_unit_added", "description": "department_admin/service admin добавил systemd-юнит в список отдела для host-service control (POST /settings/host-services/units)", "default_severity": "INFO"},
    {"action": "settings.host_service_unit_removed", "description": "department_admin/service admin убрал systemd-юнит из списка отдела для host-service control (DELETE /settings/host-services/units/{unit_id})", "default_severity": "INFO"},
    {"action": "settings.account_nopasswd_sudo_updated", "description": "Флаг «класть NOPASSWD sudoers-правило sudo-аккаунтам при provision/prepare» изменён для отдела — либо department_admin/service admin своего отдела (PUT /settings/account-nopasswd-sudo), либо account_admin для произвольного отдела (PUT /settings/account-nopasswd-sudo/departments)", "default_severity": "INFO"},
    {"action": "host_service.control", "description": "Попытка старта/стопа/рестарта systemd-юнита на отдельском хосте через POST /host/services/{unit_id}/{action} (success/denied/failure — включая неизвестный/чужой юнит и SSH/guard-отказ)", "default_severity": "CRITICAL"},
    ...
    {"action": "server.acs_snapshot_created", "description": "Worker reported ACS snapshot create outcome (callback restores busy_state to whatever it was before the ACS dispatch, regardless of outcome)", "default_severity": "CRITICAL"},
    {"action": "server.acs_snapshot_restore_done", "description": "Worker reported ACS snapshot restore outcome. On success busy_state=acs is kept and an automatic server.prepare is dispatched against the fresh image; on failure busy_state is restored to whatever it was before the ACS dispatch", "default_severity": "CRITICAL"},
    {"action": "server.acs_locked", "description": "Operation blocked because the server is currently locked by an ACS snapshot/restore in progress (busy_state=acs) and caller is not a department/service admin", "default_severity": "WARNING"},
    ...
    {"action": "vm.acquired_for_service", "description": "VM reserved on behalf of a calling service via POST /internal/vms/{id}/acquire-for-service (vms.service_busy_state, busy_service_name from the validated X-Service-Identity; status becomes 'run test')", "default_severity": "INFO"},
    {"action": "vm.reservation_taken_over", "description": "service took over a human VM booking or a testing_done reservation (acquire-for-service with takeover=true); details carry the previous holder", "default_severity": "WARNING"},
    {"action": "vm.released_for_service", "description": "service released its own VM reservation (release-for-service → free, release-for-service-as-done → testing_done)", "default_severity": "INFO"},
    {"action": "vm.service_status_changed", "description": "stage change inside the service's own VM reservation (acs → testing), service_busy_since unchanged", "default_severity": "INFO"},
    {"action": "vm.prepare_for_test", "description": "worker prepared a VM stand for a test run: virsh snapshot-revert on the hub, guest boot wait, test user provisioning, kernel/mode switch, stand setup, reboot; outcome via POST /internal/vms/{id}/prepare-for-test-done", "default_severity": "WARNING"},
    {"action": "vm.stand_setup", "description": "worker re-configured a prepared VM stand without snapshot revert (PAM fix, kernel params, test setup script, reboot); outcome via POST /internal/vms/{id}/stand-setup-done", "default_severity": "WARNING"},
    {"action": "settings.vm_test_updated", "description": "account_admin changed VM snapshot name templates used by prepare-for-test (PUT /settings/vm-test)", "default_severity": "WARNING"},
    {"action": "server.reservation_taken_over", "description": "Service took over a busy/testing_done reservation from its previous holder via POST /internal/servers/{id}/acquire-for-service with takeover=true; details carry the previous holder (busy_state, busy_user_id, busy_service_name, busy_note)", "default_severity": "WARNING"},
    {"action": "server.released_for_service", "description": "Service released its own reservation via POST /internal/servers/{id}/release-for-service (busy_state back to free) or the release-for-service-as-done variant (busy_state -> testing_done, busy_actor_type/busy_service_name/busy_note kept as context); releasing someone else's reservation is denied", "default_severity": "INFO"},
    ...
    {"action": "server.acknowledge_testing_done", "description": "Any user with server view access cleared the testing_done status back to free via POST /servers/{id}/acknowledge-testing-done; fails with SERVER_NOT_TESTING_DONE outside that state", "default_severity": "INFO"},
    ...
    {"action": "server.stand_setup_requested", "description": "testing_service asked server_service to re-configure a prepared stand without restore (POST /internal/servers/{id}/stand-setup or /internal/vms/{id}/stand-setup, target_type=vm): PAM fix, kernel params, test setup script, reboot", "default_severity": "WARNING"},
    {"action": "server.stand_setup_completed", "description": "stand-setup (without restore) reached a terminal state; failure details carry failed_step (pam_fix/stand_setup/reboot_verify)", "default_severity": "WARNING"},
    {"action": "server.stand_setup", "description": "Worker ran stand-setup steps (PAM fix, kernel params, setup script, reboot) on a prepared stand", "default_severity": "WARNING"},
    ...
    {"action": "vm.packages_installed", "description": "Packages installed in a VM guest via worker (POST /vms/{id}/packages/action → vm.install_packages): SSH apt-get/dnf/apk under sudo through the hub. Gated by (vm, vm_astra_update) + booking/lifecycle-lock", "default_severity": "WARNING"},
    {"action": "vm.packages_removed", "description": "Packages removed from a VM guest via worker (POST /vms/{id}/packages/action → vm.remove_packages): SSH apt-get/dnf/apk under sudo through the hub. Gated by (vm, vm_astra_update) + booking/lifecycle-lock", "default_severity": "WARNING"},
    {"action": "vm.packages_updated", "description": "Packages updated/upgraded in a VM guest via worker (POST /vms/{id}/packages/action → vm.update_packages): SSH apt-get/dnf/apk under sudo through the hub. Gated by (vm, vm_astra_update) + booking/lifecycle-lock", "default_severity": "INFO"},
    ...
    {"action": "vm.inventory_received", "description": "Guest inventory facts received from worker (POST /internal/vms/{id}/inventory callback): os_version (box-authoritative) / hostname / kernel / os_last_synced_at written to the VM row. Access: (server, *, inventory_submit)", "default_severity": "INFO"},
    {"action": "vm.inventory_drift_detected", "description": "Guest inventory facts diverge from the VM configuration (guest-visible vCPU differs from the configured cpu). DB config is source of truth and is NOT overwritten — drift reported with old/new", "default_severity": "WARNING"},
    {"action": "vm.users_inventory_received", "description": "Guest OS-user inventory received from worker (POST /internal/vms/{id}/users/inventory callback) and reconciled against attached accounts (server_account_vms). Access: (server_account, *, inventory_submit)", "default_severity": "INFO"},
    {"action": "vm.account_drift_detected", "description": "Guest OS-user state on a VM diverges from the DB (attributes / presence / unknown login). DB is source of truth and is NOT overwritten — only link presence (present_on_vm) is updated", "default_severity": "WARNING"},
