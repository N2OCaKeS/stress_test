...
SERVICE_EVENTS = [
    ...
    {"action": "settings.host_services_updated", "description": "account_admin изменил настройки SSH-доступа к хосту для host-service control (host/port/user/приватный ключ через PUT /settings/host-services)", "default_severity": "INFO"},
    {"action": "host_service.control", "description": "Попытка старта/стопа/рестарта ALLTA systemd-юнита на хосте через POST /host/services/{unit}/{action} (success/denied/failure — включая неизвестный юнит и SSH/guard-отказ)", "default_severity": "CRITICAL"},
