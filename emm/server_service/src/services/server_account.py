...
async def _ensure_no_linked_server_reserved(
    ...
    *,
    ...
    — кроме `busy_state=testing`, там жёстко для всех людей без исключений
    (см. `reservation.is_reserved_for_other`). Отдельно проверяем ACS-лок — на
    сервере, занятом снимком/восстановлением, проходит только админ, вне
    зависимости от того, кто держит бронь.
