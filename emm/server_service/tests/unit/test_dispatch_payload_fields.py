...
class TestInstalledPackagesPayloadSshFields:
    """installed_packages.list payload несёт host/ssh_port; сервер обязан быть prepared."""

    async def test_payload_contains_host_ssh_port(
        self, client, operator_token_a, make_server, captured_dispatch, db,
    ):
        srv = await make_server(department_id="dep_a")
        # Live-probe идёт по ключу под управляющим пользователем (после prepare).
        srv.is_managed = True
        srv.management_user = "dbos"
        await db.flush()
        await client.post(
    ...
    async def test_prepare_required_without_prepare(
        ...
        """Неподготовленный сервер — 409 PREPARE_REQUIRED, не SSH."""
        srv = await make_server(department_id="dep_a")
        ...
        assert resp.json()["error_code"] == "PREPARE_REQUIRED"
