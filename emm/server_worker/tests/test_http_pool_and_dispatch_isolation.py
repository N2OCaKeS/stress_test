"""Доборка тестов по `server_worker/tests/`.
...
class TestPasswordsApplyBmcExceptionMapping:
    ...
    async def _run(self, exc: BaseException, make_task, fetch_task, captured_audit, monkeypatch):
        ...
        tid = await make_task(
            ...
            target_server_id="srv_apply",
            payload={"server_id": "srv_apply"},
        )

        async def fake_fetch(server_id, target_department_id=None):
            return {
                "controller_id": "ipm_main",
                "kind": "idrac",
                "endpoint_url": "https://bmc.apply.test",
                "username": "root",
                "password": "old",
            }
...
class TestDispatchOutboxPerRowRollbackIsolation:
    ...
    шаблона из `tests/unit/test_dispatch_outbox_poller.py`.
    """

    async def test_middle_row_fail_does_not_rollback_neighbors(self, monkeypatch):
        from tests.unit.test_dispatch_outbox_poller import (
            _Row,
            _FakeSession,
            _install_session,
            _install_broker,
