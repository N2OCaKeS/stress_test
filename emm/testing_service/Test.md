...
| `tests/test_queue_history.py` | 7 | Комбинации фильтров, MSK-интервал, пагинация и total, повтор вне страницы, изоляция отдела, валидация периода, буквальный поиск. |
| `tests/test_audit_outbox.py` | 19 | Durable-аудит: `emit()` не ходит в сеть и кладёт событие в `audit_outbox`; запись вне request-скоупа отдельной task'ой; несериализуемый `details`; дренаж доставляет строку, держит её при недоступном loging и добивает после возврата; backoff по attempts и `Retry-After`; DLQ (permanent 4xx / missing_action / attempts_cap); программный сбой ретраится; cleanup сносит доставленные и не трогает pending; сквозной 403-запрос оставляет `http.access_denied` в таблице. |
