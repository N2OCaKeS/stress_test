...
Список сотрудников отдела, учитываемых HR-отчётом по активности. И чтение, и
запись — department-scoped: ростер несёт ФИО и Jira-аккаунты сотрудников,
чужой отдел его не видит (403 `DEPARTMENT_ISOLATION`). Запись дополнительно
под матрицей `(department_report_member, *, create|update|delete)`.
"""
...
@router.get(
    ...
    responses={403: {"description": "DEPARTMENT_ISOLATION — ростер чужого отдела."}},
)
async def list_report_members(
    ...
    items, total = await svc.list_members(
        db, identity, department_id, limit=limit, offset=offset, is_active=is_active,
    )
...
@router.get(
    ...
    responses={
        403: {"description": "DEPARTMENT_ISOLATION — сотрудник чужого отдела."},
...
async def get_report_member(
    ...
    obj = await svc.view_member(db, identity, member_id)
    return DepartmentReportMemberResponse.model_validate(obj)
