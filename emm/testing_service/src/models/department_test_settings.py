...
`activity_report_schedule` — заглушка под волну 10 (HR-отчёт по расписанию),
здесь только хранится, планировщик, который бы его читал, не реализуется.
"""
...
class DepartmentTestSettings(Base):
    ...
    activity_report_schedule: Mapped[str | None] = mapped_column(String(128), nullable=True)
