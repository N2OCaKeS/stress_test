...
async def list_by_test(db: AsyncSession, test_id: str) -> list[TestCommandArg]:
    """Все слоты одного теста (всех шагов), упорядоченные по `position`."""
    stmt = (
...
async def list_by_step(db: AsyncSession, step_id: str) -> list[TestCommandArg]:
    """Слоты одного шага теста, упорядоченные по `position`."""
    stmt = (
        select(TestCommandArg)
        .where(TestCommandArg.step_id == step_id)
        .order_by(TestCommandArg.position.asc(), TestCommandArg.id.asc())
    )
    return list((await db.execute(stmt)).scalars())


async def max_position(db: AsyncSession, step_id: str) -> int | None:
    """Максимальный `position` среди слотов шага — для append в конец."""
    stmt = select(func.max(TestCommandArg.position)).where(TestCommandArg.step_id == step_id)
    return (await db.execute(stmt)).scalar_one_or_none()


async def delete_by_step(db: AsyncSession, step_id: str) -> None:
    """Удалить все слоты шага без коммита."""
    await db.execute(sql_delete(TestCommandArg).where(TestCommandArg.step_id == step_id))
