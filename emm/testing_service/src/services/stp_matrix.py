...
**Публикация только по явному запросу, не по завершении теста.** Это то же
поведение, что у легаси: единственная точка, дёргавшая `ZefirResultTable` из
кода завершения теста, — `jira_send_status` (`backup_image.py:496-507`), и оба
её вызова закомментированы (`backup_image.py:1013,1022`). Живыми у легаси
оставались ровно ручные точки — `GET /update/<version>` (`allta_front.py:522`)
и телеграм-команда `/update_stp` (`alltabot.py:776`), которым здесь
соответствует `POST /stp/matrix/publish`. Автопубликации на терминальном
статусе кампании (рядом с `queue._maybe_post_run_summary`) не было и тут её
нет.
