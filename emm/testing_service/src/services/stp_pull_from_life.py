...
Сопоставление стенда по токену из имени идёт сначала по
`test_stands.legacy_token` (`stand3`, ровно то, чем легаси называл стенд в
имени рана), и только потом по внутреннему `test_stands.id` — для ранов,
заведённых самой EMM до того, как алиас стенду проставили. Стенд без алиаса,
которого в легаси не было, ожидаемо не найдётся — `needs_manual_mapping=True`,
а не крах. `parse_run_name` возвращает
...
async def _match_stand(
    ...
    Легаси-раны несут в четвёртом сегменте имени человеческое `stand3` —
    сначала ищем по `legacy_token`, и только потом по внутреннему id (так
    матчатся раны, заведённые самой emm до того, как алиас был проставлен).
    """
    stand = await test_stand_repo.get_by_legacy_token(db, stand_token)
    if stand is None:
