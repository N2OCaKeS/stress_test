"""Тесты хранения логов прогонов (§2.6, §8 плана миграции).

`log-chunk`/`log-segment` — internal-контракт для `testing_worker` (в будущем
поверх него строится потоковое SSH-исполнение — форма тела фиксирована,
