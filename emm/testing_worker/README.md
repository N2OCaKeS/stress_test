...
| `QUEUE_CONCURRENCY` | `4` | сколько слотов `run_polling_loop` крутится конкурентно в одном процессе (не отдельными процессами) — не меньше числа одновременно активных стендов |
