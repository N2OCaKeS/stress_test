...
/internal/queue/{id}/completed`). Тот же приём, что у `server_worker`'а для
`audit_outbox_publisher`/`heartbeat`/probe-loop'ов — фоновая task в
`TaskiqState`, а не глобальная переменная, отменяется на `WORKER_SHUTDOWN`.
