...
/**
 * `POST /test-runs` — поставить один РЦ+ядро сразу на весь явно выбранный
 * пул стендов (по закреплённому тесту на каждый, под собственным режимом
 * каждого теста). Частичные провалы не рушат остальную кампанию — см.
