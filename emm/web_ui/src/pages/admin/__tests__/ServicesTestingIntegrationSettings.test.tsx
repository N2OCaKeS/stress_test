...
import { ServicesTestingIntegrationSettings } from "@/pages/admin/services/ServicesTestingIntegrationSettings";
...
function renderPage() {
  return render(
    <MemoryRouter>
      <ThemeProvider>
        <ToastProvider>
          <ServicesTestingIntegrationSettings />
        </ToastProvider>
      </ThemeProvider>
    </MemoryRouter>,
  );
...
describe("ServicesTestingIntegrationSettings — интеграции отдела (department_integration_settings)", () => {
  beforeEach(() => {
  ...
  it("показывает блок и все поля пустыми, если строка ещё не создана", async () => {
    ...
    await waitFor(() => expect(getDepartmentIntegrationSettingsMock).toHaveBeenCalledWith("dep_1"));
