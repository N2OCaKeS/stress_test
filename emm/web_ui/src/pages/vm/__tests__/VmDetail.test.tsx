...
describe("VmDetail (диспетчер общих вкладок, live-режим)", () => {
  ...
  it("таб-бар всегда несёт все 9 вкладок при управляющей роли", async () => {
    ...
    // По умолчанию активна «Управление» — переключаемся на «Обзор», чтобы
    // проверить те же 4 секции, что у сервера (первая — «Идентификация»).
    await openTab("Обзор");
    expect(
  ...
  it("бронь вынесена в шапку карточки (как ReserveControl у сервера)", async () => {
    renderDetail(true);
    // Кнопка брони видна в шапке при управляющей роли независимо от активной
    // вкладки — «Управление» теперь дефолт и несёт свою же секцию брони,
    // поэтому одновременно видны и шапочная, и вкладочная кнопки.
    expect(
