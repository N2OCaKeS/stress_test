# Plugins for testing LDAP and HTTP performance
# Copyright (c) 2024 Alexander Bakanovskii <abakanovskii@astralinux.ru>
...
def add_arguments(parser):
    ...
    parser.add_argument('--ldap-bind-addr', type=str, default='10.191.0.150')
    parser.add_argument('--ldap-bind-password', type=str, is_secret=True, default='Astra1234')
    ...
    parser.add_argument('--kerberos-password', type=str, is_secret=True, default='Astra1234')
    ...
    # parser.add_argument("--timescale", action="store_true", env_var="LOCUST_TIMESCALE", default=False)
    # parser.add_argument("--grafana-url", type=str, env_var="LOCUST_GRAFANA_URL", default="http://localhost:3000/d/qjIIww4Zz?")
    # parser.add_argument("--pghost", type=str, env_var="PGHOST", default="localhost")
    # parser.add_argument("--pgport", type=int, env_var="PGPORT", default=5432)
    # parser.add_argument("--pgpassword", type=str, is_secret=True, env_var="PGPASSWORD", default="postgres")
    # parser.add_argument("--pguser", type=str, env_var="PGUSER", default="postgres")
    # parser.add_argument("--pgdatabase", type=str, env_var="PGDATABASE", default="")
...
# class HTTPget(FastHttpUser):
#     def wait_time(self):
#         return self.environment.parsed_options.request_delay

#     @task
#     def ldap_kinit(self):
#         self.client.ldap_kinit()
...
class LDAPClient:
    ...
    #     krb_ticket = ""
    #     try:
    #         name = gssapi.Name(f"{self.kerberos_user}@{self.kerberos_realm}", gssapi.NameType.kerberos_principal)
    #         krb_ticket = gssapi.raw.acquire_cred_with_password(name, str.encode(self.kerberos_password))
    #         response_time = (time.perf_counter() - start_perf_counter) * 1000
    #         self.environment.events.request.fire(
    #             request_type="Kerberos",
    #             name="kinit",
    ...
    #             response_length=len(str(krb_ticket)),
    #         )
    #     except (gssapi.exceptions.GSSError, Exception) as exception:
    #         response_time = (time.perf_counter() - start_perf_counter) * 1000
    #         self.environment.events.request.fire(
    #             request_type="Kerberos",
    #             name="kinit",
    ...
    # def ldap_gssapi_bind(self):
    #     start_perf_counter = time.perf_counter()
    #     ldap_krb_auth = ""
    #     try:
    #         self.krb_client = bonsai.LDAPClient(f"ldap://{self.bind_addr}")
    #         self.krb_client.set_credentials("GSSAPI", self.kerberos_user, self.kerberos_password, self.kerberos_realm)
    #         self.krb_client.set_async_connection_class(GeventLDAPConnection)
    #         self.ldap_krb_auth = self.krb_client.connect(is_async=True, timeout=self.request_timeout)
    #         response_time = (time.perf_counter() - start_perf_counter) * 1000
    #         self.environment.events.request.fire(
    #             request_type="LDAP",
    #             name="GSSAPI bind",
    ...
    #             context=None,
    #             response_time=response_time,
    #             response_length=len(str(ldap_krb_auth)),
    #         )
    #         self.ldap_krb_auth.close()
    #     except (bonsai.ConnectionError, bonsai.TimeoutError, Exception) as exception:
    #         response_time = (time.perf_counter() - start_perf_counter) * 1000
    #         self.environment.events.request.fire(
    #             request_type="LDAP",
