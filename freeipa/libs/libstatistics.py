...
from ipa_conf import CONFLUENCE_URL
...
class FreeipaStatistics:
    ...
    def get_list_required_pages(self, id_root_page):
        ...
        for id_children_from_main_page in children_main_page:
            """
                Получаем ID страниц PostgreSQL, Системные службы, Файловые системы, UnixBench, FreeIPA в каждой конкретной версии
            """
            try:
            ...
            for page_id in astra_version_child_pages:
                ...
                """
                    Проверяем есть ли в заголовке FreeIPA и имеются ли дочерние страницы
                """
                if "FreeIPA" in page.get("title") and self.CP.get_child_page_as_html(id=page_id):
                ...
                if "1.7" in name_child_page or "1.8" in name_child_page:
                    ...
                    if hz_kak_nazvat_pages:
                        for item_page in hz_kak_nazvat_pages:
                            if "FreeIPA" in self.CP.get_page_as_html(id=item_page).get("title"):
    ...
    def get_info_from_pages(self, pages, columns_df, rc=False, version_key=None):
        ...
        def collect_data(test_name=None):
            data_for_df = {
    ...
    """
    def upload_statistics(self, type_stat='FreeIPA', rc=None, pp_title=None):
        if rc and pp_title:
...
if __name__ == '__main__':
    ...
    stat = FreeipaStatistics(username=args.USER, token=args.TOKEN)
