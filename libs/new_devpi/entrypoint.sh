...
LOCAL_UPLOAD_USER="${DEVPI_UPLOAD_USER:-allta}"
LOCAL_UPLOAD_PASSWORD="${DEVPI_UPLOAD_PASSWORD:-}"
...
if [ -n "$LOCAL_UPLOAD_USER" ] && [ -z "$LOCAL_UPLOAD_PASSWORD" ]; then
    echo "нужно задать DEVPI_UPLOAD_PASSWORD для локального пользователя $LOCAL_UPLOAD_USER" >&2
    exit 1
fi
