    ...
    def __init__(self,
        ...
        with open("{}/{}".format(self.__report_dir, REPORT_FILENAME), 'r') as file:
            ...
            for test, regexp in REGEXP_PARSERS.items():  # ищем совпаденяи по тестам
                ...
                self.__raw_dict[test]['parallel_threads'] = [float(index) for index in range(self.__ox_upper_limit)]
                print('parallel_threads', [float(index) for index in range(self.__ox_upper_limit)])
