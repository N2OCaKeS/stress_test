...
from libs.liblsb import response
from lsb_conf import JIRA_URL, CONFLUENCE_URL


class UploaderZC(Public, UnixBenchStatistics):
    ...
    def test_cycle_status_changer(self, status):
        ...
        if self.statistics == True:
            statistics = UnixBenchStatistics(username=self.UN, 
                                             token=self.CT)
...
class ZefirResultTable:
    ...
    def __init__(self,
        ...
        if response_times.status_code == 200:
            with open('./template/times.html', 'wb') as tfb:
                tfb.write(response_times.content)
        else:
            with open('./template/times.html', 'w') as f:
        ...
        if response_stand.status_code == 200:
            with open('./template/stand.html', 'wb') as sfb:
                sfb.write(response_stand.content)
        else:
            with open('./template/stand.html', 'w') as f:
        ...
        with open('/home/u/git/stress_test/linux_system/template/stand.html', 'r') as r:
            stand = r.read()
        with open('/home/u/git/stress_test/linux_system/template/times.html', 'r') as r:
            times = r.read()
