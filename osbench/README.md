...
**lat_ctx** — аппаратно-зависимый тест. Он показывает возможности процессора, а не оптимизации ОС.    
Используется как контрольный тест для проверки, что система работает корректно на аппаратном уровне.  
Если фиксируются аномальные значения на какой-то ОС, это сигнал о проблемах с планировщиком или настройками ядра. 
Если на разных ОС результаты одинаковые, значит различия в других тестах вызваны именно ОС, а не железом.      
    
**Примечание:** тест используется в диагностических целях и не оказывает существенного влияния на итоговый рейтинг.   
...
1. **Фокус на ОС** — преимущественно измеряет характеристики системы, а не железа
