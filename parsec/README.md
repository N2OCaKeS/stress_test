...
### Test 3
### DIGSIG. Check digsig time
Производится оценка количества затраченного времени на проверку хеша неподписанного файла.    

Нагрузка обеспечивается путем проверки 4096 обращений к неподписанному файлу в 10 потоков.
Сравнивается время на проверку хеша подписанных и неподписанных файлов.  
Выполняется в режиме **смоленск**.      


### Test 4
### Spinlock_impact_UnixBench_test
Производится оценка влияния спин-локов ядра на производительность CPU под нагрузкой UnixBench.

В тесте производится расчет суммарного и детального CPU-времени, затраченного на выполнение функций спин-локов ядра,
путем параллельного профилирования утилитой perf в системном режиме (`-a`) на протяжении всего прогона UnixBench.
Список отслеживаемых функций получается автоматически из `/proc/kallsyms` по шаблону:
`_raw_spin_lock*`, `queued_spin_lock*`, `_raw_spin_trylock*`.
Выполняется в режиме **смоленск**.
