...
from ps_conf import CONFLUENCE_URL
...
class ParsecStatistics:
    ...
    def get_list_required_pages(self, id_root_page):
        ...
        for id_children_from_main_page in children_main_page:
            ...
            for page_id in astra_version_child_pages:
                ...
                """
                if "Parsec" in page.get("title") and self.CP.get_child_page_as_html(id=page_id):
                ...
                if "1.7" in name_child_page or "1.8" in name_child_page:
                    ...
                    if hz_kak_nazvat_pages:
                        for item_page in hz_kak_nazvat_pages:
                            if "Parsec" in self.CP.get_page_as_html(id=item_page).get("title"):
    ...
    def get_info_from_pages(self, pages, columns_df, rc=False, version_key=None):
            ...
            for id in pages:
                ...
                for title in list_pages_with_report:
                    ...
                    if temp_data[0] == test_name:
                        ...
                        """
                        try:
                            if test_name == "parsec impact-fs" or test_name == "parsec impact-fs aud-off":
                                total_used_by_parsec_func = soup.find(string='Total used by parsec func').next_element.next_element.text
                        ...
                        if stand == "stand3":
                            if test_name == "parsec impact-fs" or test_name == "parsec impact-fs aud-off":
                                total_used_by_parsec_func_with_link = f'<a href="{link}">{total_used_by_parsec_func}</a>'
                                data_for_df["stand3"]["data"].append([astra_version, kernel, sec_mode, stand, total_used_by_parsec_func_with_link, float(total_used_by_parsec_func)])
                            else:
        ...
        def build_dataframes(data_for_df, test_name):
            ...
            for key, data in data_for_df.items():
                ...
                dataframes_for_summary_graph[key] = df[['Релиз', 'Ядро', 'Стенд', 'Total used by parsec func', 'rating_2']]
        ...
        def create_graphs(data_for_df, test_name, temp_data_for_graph):
            ...
            for key, data_ratings in data_for_df.items():
                ...
                ax.set_title(f"Сравнительная диаграмма значений используемых функций модуля Parsec.\n{test_name}")
                ...
                ax.set_ylabel("Процент (%) использования функций модуля Parsec")
        ...
        def create_line_graph(data_for_df):
            temp_data_for_graph = {}
            for key, data in data_for_df.items():
                if len(data.get("data")) == 0:
                    continue
                ...
                ax.plot(shcala, df['Total used by parsec func'], "o-", color='#ea5c76')
                ax.set_xticks(shcala)
                plt.gca().set_xticklabels(shcala_text, rotation=20, horizontalalignment='right')
                # Lighten borders
                ...
                fig.savefig(f"{stat_dir}/line_graph_{key}.jpg")

        def create_line_graph_summ(lst_data):
            # TODO Работает только для stand3
            try:
                merged_df = pd.merge(lst_data[0]['stand3'], lst_data[1]['stand3'], on=["Релиз", "Стенд", "Ядро"], how='outer')
            except (IndexError, KeyError, pd.errors.MergeError):
                return False
            ...
            ax.grid(True, alpha=.6)
            ax.set_ylabel("Процент (%) использования функций модуля Parsec")
            ax.set_title("Линейная диаграма сравнения используемых функций модуля Parsec\n")
            colors = ['#f90829', '#007b7a', '#f9b312', '#c7d84c']
            shcala_text = (merged_df['Релиз'] + "_" + merged_df['Ядро'])
            shcala = [x for x in range(1, len(merged_df) + 1, 1)]
            for index in range(ratings_for_plt_graph.shape[1]):
                ax.plot(shcala, ratings_for_plt_graph.iloc[::, index], "o-", color=colors[index])
            ax.set_xticks(shcala)
            plt.gca().set_xticklabels(shcala_text, rotation=20, horizontalalignment='right')
            plt.legend(['smolensk', 'aud off'])
            # Lighten borders
            plt.gca().spines["top"].set_alpha(.0)
            plt.gca().spines["bottom"].set_alpha(.3)
            plt.gca().spines["right"].set_alpha(.0)
            ...
            plt.savefig(f"{stat_dir}/line_graph.jpg")

            """
            Строим HTML
            """
            merged_df = merged_df.rename(columns={
                'Total used by parsec func_x': 'Total used by parsec func',
                'Total used by parsec func_y': 'Total used by parsec func aud off'
            })
            statistics_table_html = merged_df[['Релиз', 'Ядро', 'Стенд', 'Total used by parsec func', 'Total used by parsec func aud off']].fillna("-").to_html(escape=False, index=False)
            file_html = open(f"{stat_dir}/table_0.html", "w")
            file_html.writelines(f'<h1><a href="https://{CONFLUENCE_URL}/pages/viewpage.action?pageId=192234259">Описание стендов нагрузочного тестирования</a></h1>')
            # grage = self.get_grade(key)
        ...
        data_df = collect_data(test_name="parsec impact-fs")
        tmp_data_for_gr, tmp_data_krnl, df_psql = build_dataframes(data_for_df=data_df, test_name="parsec impact-fs")
        create_graphs(data_for_df=data_df, test_name="parsec impact-fs", temp_data_for_graph=tmp_data_for_gr)

        data_df_aud_off = collect_data(test_name="parsec impact-fs aud-off")
        tmp_data_for_gr, tmp_data_krnl, df_psql_aud_off = build_dataframes(data_for_df=data_df_aud_off, test_name="parsec impact-fs aud-off")
        create_graphs(data_for_df=data_df_aud_off, test_name="parsec impact-fs aud-off", temp_data_for_graph=tmp_data_for_gr)

        res = create_line_graph_summ(lst_data=[df_psql, df_psql_aud_off])
        if res == False:
            create_line_graph(data_df)

    """
        Создаем итоговую html страницу для life
    """
    def upload_statistics(self, type_stat='Parsec', rc=None, pp_title=None):
        if rc and pp_title:
        ...
        for file in sorted(os.listdir(f"{stat_dir}"))[::-1]:
            ...
            if file.endswith("jpg"):
                ...
                img = template_img.format(page_id=confluence_stat.get_confluence_page_id(SPACE, f"Статистика.{page_rc_title} {type_stat}"),
                                                    img_png=file, CONFLUENCE_URL=CONFLUENCE_URL)
                image_lst_line_graph.append(img)
            
        html_list = []
        html_list.append(image_lst_line_graph[0])
        
        try:
            html_list.append(table_main[0])
        except IndexError:
            pass
    ...
    def update_statistics(self):
        ...
        columns = ["Релиз", "Ядро", "Режим защищенности", "Стенд", "Total used by parsec func", 'rating_2']
...
if __name__ == '__main__':
    ...
    stat = ParsecStatistics(username=args.USER, token=args.TOKEN)
