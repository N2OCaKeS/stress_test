...
GRANT SELECT, INSERT, DELETE ON s1.t1 TO Администратор_БД;
-- Роли Администратор_БД разрешено изменение поля classificator
GRANT UPDATE (classificator) ON s1.t1 TO Администратор_БД;
-- Роли Пользователь разрешено чтение записей
GRANT SELECT ON s1.t1 TO Пользователь;
-- Роли Пользователь разрешено изменение полей update_user, update_date
GRANT UPDATE (update_user, update_date) ON s1.t1 TO Пользователь;
...
CREATE POLICY org_policy_01 ON s1.t1 FOR ALL TO PUBLIC
...
CREATE POLICY org_policy_11 ON s1.t1 FOR ALL TO PUBLIC
