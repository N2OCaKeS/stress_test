...
case "$MODE" in
  OVER)
    if [ "$OLD_MASTER_ID" -ne "$NEW_MASTER_ID" ] && [ "$NEW_MASTER_ID" -ge 0 ]; then
      ...
      for node in "${DATABASE_NODES[@]}"; do
        ...
        if [ "$node_fqdn" != "$NEW_MASTER_HOST" ] && [ "$node_fqdn" != "$OLD_MASTER_HOST" ]; then
          echo "[$(date)] Updating postgresql.auto.conf on $node_fqdn..." >> "$LOGFILE"
          SSH_NODE="$SSHPASS ssh $SSH_OPTS $USERNAME@$node_fqdn"
          $SSH_NODE "cd /tmp && \
            sudo sed -E -i 's|(host=)[^[:space:]]+|\1${NEW_MASTER_HOST}|' ${NEW_MASTER_DATA}/postgresql.auto.conf && \
    ...
    ;;
  BACK) # TODO разобраться как вызывается failback
