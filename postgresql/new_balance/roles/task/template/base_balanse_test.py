...
for c in $clients; do
    ...
    echo "12345678" | sudo kinit user0 
    pgbench -h pgpool.balance.rbt -p 5440 -U user0@BALANCE.RBT --random-seed=13 -T $t -j $c -c $c contrprimer >> "${dir}/pgbench_result.txt"
...
def test_run(clients, repeat):
    ...
    #Настройка и запуск
    with open('start_test.sh', 'w') as test_script:
        test_script.write(script)
    chmod('start_test.sh', 0o755)
