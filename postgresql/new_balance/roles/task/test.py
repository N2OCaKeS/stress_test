...
class Test:
    ...
    def test(self):
        ...
        provider.scp(
            scp_settings=scp, vms_dates=VMS_DATES, username=USERNAME, password=PASSWORD
        )
        ...
        test = {
            ...
            "lbdb1": {
                ...
                "reload conf3": {
                    "command": f"sleep 1 && sudo PCPPASSFILE=/tmp/.pcppass pcp_reload_config -w -h {PGPOOL_HOSTNAME} -U {PGPOOL_PCP_USER} --scope=cluster",
                    "signal set": "reload conf3",
                    "signal get": ["disable load balancing db0"],
                },
                "detach_db0": {
                    "command": f"sleep 1 && sudo PCPPASSFILE=/tmp/.pcppass pcp_detach_node -w -U {PGPOOL_PCP_USER} -h {PGPOOL_HOSTNAME} 0",
                    "signal set": "detach db0",
                    "signal get": ["reload conf3"],
                },
                "atach_db0": {
                    "command": f"sleep 1 && sudo LC_ALL=C LANG=C PCPPASSFILE=/tmp/.pcppass pcp_attach_node -w -U {PGPOOL_PCP_USER} -h {PGPOOL_HOSTNAME} 0",
