...
sed -i "s/^#\?\s*max_connections\s*=.*/max_connections = 500/" /etc/postgresql/$PG_VERSION/$PG_SETEST_CLUSTER/postgresql.conf
sed -i "s/^#\?\s*shared_buffers\s*=.*/shared_buffers = 64512MB/" /etc/postgresql/$PG_VERSION/$PG_SETEST_CLUSTER/postgresql.conf
sed -i "s/^#\?\s*effective_cache_size\s*=.*/effective_cache_size = 193536MB/" /etc/postgresql/$PG_VERSION/$PG_SETEST_CLUSTER/postgresql.conf
...
if [[ "$PG_VERSION" -eq "14" ]]; then
  ...
  sed -i "s/^#\?\s*work_mem\s*=.*/work_mem = 82574kB/" /etc/postgresql/$PG_VERSION/$PG_SETEST_CLUSTER/postgresql.conf
fi
...
# sed -i "s/^#\?\s*max_connections\s*=.*/max_connections = 200/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*shared_buffers\s*=.*/shared_buffers = 8GB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*effective_cache_size\s*=.*/effective_cache_size = 24GB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*maintenance_work_mem\s*=.*/maintenance_work_mem = 2GB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*checkpoint_completion_target\s*=.*/checkpoint_completion_target = 0.9/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*wal_buffers\s*=.*/wal_buffers = 16MB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*default_statistics_target\s*=.*/default_statistics_target = 100/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*random_page_cost\s*=.*/random_page_cost = 1.1/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*effective_io_concurrency\s*=.*/effective_io_concurrency = 200/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*work_mem\s*=.*/work_mem = 10485kB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*min_wal_size\s*=.*/min_wal_size = 1GB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*max_wal_size\s*=.*/max_wal_size = 4GB/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*max_worker_processes\s*=.*/max_worker_processes = 8/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*max_parallel_workers_per_gather\s*=.*/max_parallel_workers_per_gather = 4/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*max_parallel_workers\s*=.*/max_parallel_workers = 8/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
# sed -i "s/^#\?\s*max_parallel_maintenance_workers\s*=.*/max_parallel_maintenance_workers = 4/" /etc/postgresql/$PG_VERSION/$PG_FILES_CLUSTER/postgresql.conf
