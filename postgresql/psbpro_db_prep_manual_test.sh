...
if [[ "$1" == "host" ]]; then
    ...
    sed -i 's/#ac_enable_maclabels_on_files.*$/ac_enable_maclabels_on_files = true/' "$CONF_DIR"
    sed -i 's/max_connections.*/max_connections = 40/' "$CONF_DIR"
    sed -i 's/shared_buffers.*/shared_buffers = 8GB/' "$CONF_DIR"
    sed -i 's/effective_cache_size.*/effective_cache_size = 24GB/' "$CONF_DIR"
    sed -i 's/maintenance_work_mem.*/maintenance_work_mem = 2GB/' "$CONF_DIR"
    sed -i 's/checkpoint_completion_target.*/checkpoint_completion_target = 0.9/' "$CONF_DIR"
    sed -i 's/wal_buffers.*/wal_buffers = 16MB/' "$CONF_DIR"
    sed -i 's/default_statistics_target.*/default_statistics_target = 500/' "$CONF_DIR"
    sed -i 's/random_page_cost.*/random_page_cost = 1.1/' "$CONF_DIR"
    sed -i 's/effective_io_concurrency.*/effective_io_concurrency = 300/' "$CONF_DIR"
    sed -i 's/work_mem.*/work_mem = 26214kB/' "$CONF_DIR"
    sed -i 's/min_wal_size.*/min_wal_size = 4GB/' "$CONF_DIR"
    sed -i 's/max_wal_size.*/max_wal_size = 16GB/' "$CONF_DIR"
    sed -i 's/max_worker_processes.*/max_worker_processes = 8/' "$CONF_DIR"
    sed -i 's/max_parallel_workers_per_gather.*/max_parallel_workers_per_gather = 4/' "$CONF_DIR"
