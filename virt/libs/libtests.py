...
class LargeFio(CreateVM):
    ...
    def start_test(self):
        ...
        print(check_output_command(f"sudo qemu-img create -f qcow2 -o cluster_size=1M {add_disk_path}/largefio.qcow2 1T"))
        ...
        send_remote_command(command="sudo apt-get update && sudo apt-get install -y parted e2fsprogs util-linux",
                            ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
        send_remote_command(command="sudo parted -s /dev/vdb mklabel gpt", ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
        send_remote_command(command="sudo parted -s /dev/vdb mkpart primary ext4 0% 100%", ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
        send_remote_command(command="sudo partprobe /dev/vdb", ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
        send_remote_command(command="sudo udevadm settle", ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
        send_remote_command(command="sudo mkfs.ext4 -F /dev/vdb1", ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
        send_remote_command(command="sudo dd if=/dev/urandom of=/dev/vdb1 oflag=direct bs=1G count=2048", ip=self.vm_dates['testvm1']['ip'], user=self.user, password=self.password)
